<script>
    ...
    // Função para escapar caracteres perigosos (XSS)
    ...
    async function carregarLivros() {
        ...
        if (Array.isArray(livros)) {
            livros.forEach(livro => {
                ...
                li.className = 'livro';
                li.innerHTML = `
                    <strong>${escapeHTML(livro.name)}</strong> - ${escapeHTML(livro.category)}
                    <button onclick="prepararEdicao(${livro.id}, '${escapeHTML(livro.name)}', '${escapeHTML(livro.category)}')">Editar</button>
